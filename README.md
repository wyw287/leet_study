...
### `leet solution`:卡住了就看参考解

```bash
leet solution 05        # 省略题号则取最近编辑过的解答
```

它打印 `problems/<id>/optimal.cu|py` —— 一份**能达到目标评级**的实现,带注释讲清
为什么这样写快、试过哪些死路。

如果你还没通过这道题,它会加一行提示,但**不拦着你**:

```
你还没通过这道题 —— 先自己试过再看,收获会大得多。
```

参考解还有个不显眼但更重要的作用:**它是 `leet validate` 证明门槛可达的依据**。
有一份达到 S 级的参考解,才谈得上"这个门槛真的拿得到" —— 见下。

### 门槛必须有一份参考解背书

`leet validate` 会跑一遍参考解,确认它达到目标评级。够不到就报错:

```
✗ 参考解能证明门槛可达   参考解只拿到 B 级(2.14x)—— 说明门槛设高了,
                        存在不了这样的实现;请按实测下调 perf.grades
```

这条检查抓到过一个真问题:`05-conv2d` 原本的 S=2.9x 是**死门槛** ——
它需要 117µs,而该用例的物理下限是 133µs(134MB 必经流量 @ 1008 GB/s),
等于凭空要 13% 的额外带宽。当初的推算写成「134MB/119µs ≈ 1.1 TB/s」,
而 1.1 TB/s 本身就超过这张卡的标称峰值 —— 推理链是错的,却一直没人发现,
因为**没有任何东西去验证"这个门槛真的达得到"**。现在有了。
